...
#         self.sceneids.extend(sceneids.cpu().numpy())
#         self.pids.extend(pid.cpu().numpy())
#     # def update(self, output):
#     #     feat, pid, camid, sceneid, img_path = output
#     #     self.feats.append(feat)
#     #     # 关键修复：cuda tensor -> cpu -> 纯数值，再转numpy
#     #     pid = pid.detach().cpu() if torch.is_tensor(pid) else pid
#     #     camid = camid.detach().cpu() if torch.is_tensor(camid) else camid
#     #     sceneid = sceneid.detach().cpu() if torch.is_tensor(sceneid) else sceneid
...
class R1_mAP_eval():
    def __init__(self, num_query, max_rank=50, feat_norm=True, reranking=False):
        self.num_query, self.max_rank, self.feat_norm, self.reranking = num_query, max_rank, feat_norm, reranking
        self.reset()
