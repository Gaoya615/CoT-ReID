...
def re_ranking(probFea, galFea, k1, k2, lambda_value, local_distmat=None, only_local=False):
    ...
    # Clamp tiny negative squared distances caused by floating point error and
    # protect constant columns from division by zero.
    original_dist = np.maximum(original_dist, 0.0).astype(np.float32, copy=False)
    column_max = np.maximum(np.max(original_dist, axis=0), 1e-12)
