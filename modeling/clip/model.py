...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        x, atten_map = self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)
        return x
...
class CLIP(nn.Module):
    def __init__(self, cfg,
        ...
        if self.num_text_prompt > 0:
            self.text_prompt = nn.Parameter(torch.randn(self.num_text_prompt, transformer_width))
        else:
...
# # #1. 提取每个样本中“分数最高的token”的特征（压缩序列维度T）
# #         x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
# #        x = x.permute(1, 0, 2)
#         x = self.ln_final(x).type(self.dtype)
        
#         x = x @ self.text_projection
#       #32,77,512  print(x.shape)
#         return x

    # def encode_text(self, text, modality=None,image_inverse=None):
    #     x = self.token_embedding(text).type(self.dtype)
    #     # if self.text_prompt is not None:
    #     text_prompt = self.text_prompt.repeat(text.shape[0], 1, 1)
    #         # if image_inverse is not None:
    #         #     x[:, 6:6 + self.num_text_prompt] = x[:, 6:6 + self.num_text_prompt] + text_prompt
    #         # else:
    #     x[:, 5:5 + self.num_text_prompt] = x[:, 5:5 + self.num_text_prompt] + text_prompt
    #     # if image_inverse is not None:
    #     #     x[:, 5] = x[:, 5] + image_inverse
    #     x = x + self.positional_embedding.type(self.dtype)
    #     x = x.permute(1, 0, 2)
    #   #  for i in range(len(self.transformer.resblocks)):
    ...
    #     x = self.transformer(x)
    #             # else:
    #             #     x = self.transformer.resblocks[i](x, modality, i, last_prompt, prompt_sign=True,
    #             #                                                    adapter_sign=True)
    ...
    def encode_text(self, text):
        if text.ndim != 2:
            raise ValueError(f"encode_text expects [batch, tokens], got {tuple(text.shape)}")

        # Keep the pretrained 77-token CLIP transformer unchanged.  Complete
        # inputs are split into [SOT, 75 content tokens, EOT] windows and all
        # windows belonging to an item are averaged.
        if text.shape[1] > self.context_length:
            sot_token, eot_token = 49406, 49407
            windows, owners = [], []
            content_length = self.context_length - 2
            for batch_idx, row in enumerate(text):
                valid = row[row.ne(0)]
                if valid.numel() and valid[0].item() == sot_token:
                    valid = valid[1:]
                if valid.numel() and valid[-1].item() == eot_token:
                    valid = valid[:-1]
                for chunk in valid.split(content_length):
                    window = row.new_zeros(self.context_length)
                    window[0] = sot_token
                    window[1:1 + chunk.numel()] = chunk
                    window[1 + chunk.numel()] = eot_token
                    windows.append(window)
                    owners.append(batch_idx)

            window_text = torch.stack(windows)
            encoded = self._encode_text_window(window_text)
            owner_index = torch.tensor(owners, device=text.device, dtype=torch.long)
            output = encoded.new_zeros(text.shape[0], self.context_length, encoded.shape[-1])
            counts = encoded.new_zeros(text.shape[0], 1, 1)
            output.index_add_(0, owner_index, encoded)
            counts.index_add_(0, owner_index, encoded.new_ones(len(owners), 1, 1))
            output = output / counts.clamp_min(1)

            eot_positions = window_text.argmax(dim=-1)
            global_chunks = encoded[
                torch.arange(encoded.shape[0], device=text.device), eot_positions
            ]
            global_output = encoded.new_zeros(text.shape[0], encoded.shape[-1])
            global_output.index_add_(0, owner_index, global_chunks)
            output[:, 0] = global_output / counts[:, 0].clamp_min(1)
            return output

        return self._encode_text_window(text)

    def _encode_text_window(self, text):
        x = self.token_embedding(text).type(self.dtype)
