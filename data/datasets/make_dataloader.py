...
from .bases import LongTextCoTImageDataset as ImageDataset
...
def train_collate_fn(batch):
    """
    """
    imgs, pids, camids, viewids, indices, img_paths, r_text, n_text, t_text, r_cot, n_cot, t_cot = zip(*batch)
    ...
    def stack_text(items):
        return torch.nn.utils.rnn.pad_sequence(items, batch_first=True)

    text = {'rgb_text': stack_text(r_text),
            'ni_text': stack_text(n_text),
            'ti_text': stack_text(t_text),
            'r_cot': stack_text(r_cot),
            'n_cot': stack_text(n_cot),
            't_cot': stack_text(t_cot)}
    return imgs, pids, camids, viewids, indices, img_paths, text


def val_collate_fn(batch):
    imgs, pids, camids, viewids, _, img_paths,r_text, n_text, t_text, r_cot, n_cot, t_cot = zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)  # 新增：将元组转为张量
    viewids = torch.tensor(viewids, dtype=torch.int64)
